...
fn constructor() {
    ...
    let vesting_escrow: Key = runtime::get_named_arg("vesting_escrow");

    mappings::set_key(&mappings::self_hash_key(), contract_hash);
    mappings::set_key(&mappings::self_package_key(), package_hash);
    mappings::set_key(
        &mappings::vesting_escrow_key(),
        ContractHash::from(vesting_escrow.into_hash().unwrap_or_default()),
    );
}

#[no_mangle]
fn transfer() {
    let vesting_escrow_address: ContractHash = mappings::get_key(&mappings::vesting_escrow_key());
    ...
    let ret: Result<(), u32> = runtime::call_contract(vesting_escrow_address, "transfer", args);
...
fn approve() {
    ...
    let _ret: () = runtime::call_contract(vesting_escrow_address, "approve", args);
}
...
fn allowance() {
    ...
    let ret: U256 = runtime::call_contract(vesting_escrow_address, "allowance", args);
...
fn get_entry_points() -> EntryPoints {
    ...
    entry_points.add_entry_point(EntryPoint::new(
        ...
        vec![
            ...
            Parameter::new("vesting_escrow", Key::cl_type()),
        ],
        <()>::cl_type(),
